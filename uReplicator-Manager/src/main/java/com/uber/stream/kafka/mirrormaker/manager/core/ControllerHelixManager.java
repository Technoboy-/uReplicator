...
public class ControllerHelixManager implements IHelixManager {
  ...
  public InstanceTopicPartitionHolder createNewRoute(String pipeline, int routeId) throws Exception {
    ...
    if (!isPipelineExisted(pipeline)) {
      ...
      LOGGER.info("Create new pipeline {} partition {} to instance {}", pipeline, routeId, instance);
  ...
  public boolean isAutoBalancingEnabled() {
    ...
    if (config.containsKey(AUTO_BALANCING) && config.get(AUTO_BALANCING).equalsIgnoreCase(DISABLE)) {
      return false;
    }
    return true;
  }
  ...
  private HelixConfigScope newClusterConfigScope(){
    return new HelixConfigScopeBuilder(ConfigScopeProperty.CLUSTER).forCluster(_helixClusterName).build();
  }
